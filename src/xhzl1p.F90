...
SUBROUTINE XHZL1P(M,N, H,LDH, JVEC, S,LDS, Z,LDZ, JS,JSPAIR, NSWP,CPR,&
  ...
  !DIR$ ASSUME (MOD(LDH,CALIGN) .EQ. 0)
  !DIR$ ASSUME (MOD(LDS,CALIGN) .EQ. 0)
  !DIR$ ASSUME (MOD(LDZ,CALIGN) .EQ. 0)
  !DIR$ ASSUME (NSWP .GE. 0)
  !DIR$ ASSUME (CPR .GE. 1)
#else
  IF (M .LT. 0) STOP 'XHZL1: M .LT. 0'
  IF (N .LT. 0) STOP 'XHZL1: N .LT. 0'
  IF (M .LT. N) STOP 'XHZL1: M .LT. N'
  IF (MOD(N,2) .NE. 0) STOP 'XHZL1: MOD(N,2) .NE. 0'
  IF (MOD(LDH,CALIGN) .NE. 0) STOP 'XHZL1: MOD(LDH,CALIGN) .NE. 0'
  IF (MOD(LDS,CALIGN) .NE. 0) STOP 'XHZL1: MOD(LDS,CALIGN) .NE. 0'
  IF (MOD(LDZ,CALIGN) .NE. 0) STOP 'XHZL1: MOD(LDZ,CALIGN) .NE. 0'
  IF (NSWP .LT. 0) STOP 'XHZL1: NSWP .LT. 0'
  IF (CPR .LT. 1) STOP 'XHZL1: CPR .LT. 1'
#endif
  ...
  DO SWEEP = 1, NSWP
     ...
     SNROT = 0
     DO STEP = 1, NSTEPS
        !$OMP  PARALLEL DO DEFAULT(NONE) NUM_THREADS(CPR)                                &
              ...
              IF (PAIR .LE. NPAIRS) THEN
                 ...
                 DO I = 1, M, DSIMDL
                    ...
                    DO J = 1, L
                       ZTMP1(J) = H(I+(J-1),P)
                       ZTMP2(J) = H(I+(J-1),Q)
                       DTMP1(J) = DTMP1(J) + JVEC(M) * (REAL(ZTMP1(J))*REAL(ZTMP1(J)) + AIMAG(ZTMP1(J))*AIMAG(ZTMP1(J)))
                       DTMP2(J) = DTMP2(J) + JVEC(M) * (REAL(ZTMP2(J))*REAL(ZTMP2(J)) + AIMAG(ZTMP2(J))*AIMAG(ZTMP2(J)))
                       DTMP3(J) = DTMP3(J) + JVEC(M) * (REAL(ZTMP1(J))*REAL(ZTMP2(J)) + AIMAG(ZTMP1(J))*AIMAG(ZTMP2(J)))
                       DTMP4(J) = DTMP4(J) + JVEC(M) * (REAL(ZTMP1(J))*AIMAG(ZTMP2(J))- AIMAG(ZTMP1(J))*REAL(ZTMP2(J)))
                    END DO
                 END DO
  ...
  IF (NROT(1) .GT. 0) THEN
     ...
     DO J = 1, N
        ...
        DO I = 1, M, DSIMDL
           ...
           DO L = 1, P
              ZTMP1(L) = H(I+(L-1),J)
              DTMP1(L) = DTMP1(L) + JVEC(M) * (REAL(ZTMP1(L))*REAL(ZTMP1(L)) + AIMAG(ZTMP1(L))*AIMAG(ZTMP1(L)))
           END DO
        END DO
        DTOL(1) = SUM(DTMP1)
